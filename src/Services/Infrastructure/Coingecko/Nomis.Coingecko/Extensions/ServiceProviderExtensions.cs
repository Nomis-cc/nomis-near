            ...
            this IServiceProvider provider)
        {
            if (provider.GetService(typeof(CoingeckoService)) == null)
            {
                throw new InvalidOperationException(
                    $"Unable to find {nameof(CoingeckoService)} service in registered services. You should use {nameof(ServiceCollectionExtensions.AddCoingeckoService)} in ConfigureServices(...)");
            }
